...
class Table_Node1{
	public String item;
	public int count;
	public List<Node1> nextNodes;
	public Table_Node1() {
		
	}
	public Table_Node1(String it,int c) {
		item=it;
		count=c;
		nextNodes=new ArrayList<>();
	}
	public List<Table_Node1> createTable_Node ( LinkedHashMap<List<String>,Integer> C) {
		List<Table_Node1> t=new ArrayList<>();
		for(List<String> l:C.keySet()) {
			t.add(new Table_Node1(l.get(0),C.get(l)));
		}
		return t;
	}
}
...
class FPTree1 {
		 ...
		 if (!T.children.isEmpty()) {
	            for (Node1 child : T.children) {
	                if (child.item.equals(t.get(0))) {
	                    ...
	                    t.remove(t.get(0));
	                    if (!t.isEmpty()) {
	                        insert(child, t,table);
	                    }
	                    return;
	                }
	            }
	            Node1 newNode = new Node1(t.get(0));
	            for(Table_Node1 tab:table) {
	...
	boolean isSubList(List<String> l1,List<String> l2) {
		boolean flag=true;
	...
	void makeTree(Node1 T,List<List<String>> trans,List<Table_Node1> table) {
		for(int i=0;i<trans.size();i++) {	
			insert(T,trans.get(i),table);
		}
	}
	...
	boolean onlySinglePath(Node1 tree) {
	...
	LinkedHashMap<Set<String>, Integer> constructCondiBasePatt(List<Table_Node1> table,Node1 tree,List<String> b) {
		LinkedHashMap<Set<String>, Integer> cfp=new LinkedHashMap<>();
		for(int i=0;i<table.size();i++) {
			Table_Node1 tab=table.get(i);
			if(b.get(0).equals(tab.item)) {
				int cou=tab.nextNodes.get(0).count;
				for(Node1 n:tab.nextNodes) {
					cou=n.count;
					Set<String> lst=new LinkedHashSet<>();
					n=n.parent;
					while(n.parent!=null) {
	...
	void printTable(List<Table_Node1> table) {
		for(int i=0;i<table.size();i++) {
			System.out.println(table.get(i).item+" "+table.get(i).count+" "+table.get(i).nextNodes.size());
		}
	}
	void FpGrowth(Node1 tree,List<Table_Node1> table,List<String> a,LinkedHashMap<List<String>,Integer> freqitems,int sup) {
		...
		if(onlySinglePath(tree)) {
			...
			for(int i=table.size()-1;i>=0;i--) {
				...
				/*create f p tree*/
				Table_Node1 t=new Table_Node1();
				List<Table_Node1> table1=t.createTable_Node(C);
				Node1 x=new Node1("X");
...
public class FPGrowth1{
	public static void main(String[] args) throws Exception {
		FPTree1 f=new FPTree1();/*object of FPTree class*/
		Scanner sc=new Scanner(System.in);
		...
		/*create f p tree*/
		Table_Node1 t=new Table_Node1();
		List<Table_Node1> table=t.createTable_Node(C);
